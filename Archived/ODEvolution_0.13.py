...
class Experiment:
    ...
    def dilute(self):
        ...
        self.gluc = np.append(self.gluc_start)
    ...
    def kill(self):
        ...
        self.append_genos('n', np.where(diluted>=1, diluted, 0))
    ...
    def conduct_once(self):
        ...
        y0 = np.append(self.from_genos('n')[-1], self.gluc[-1])

        self.sol = solve_ivp(self.pop_ivp_mut, self.t_span, y0=y0, args=([self.ode_args]),
                                 dense_output=True, 
                                 # events =no_gluc,
