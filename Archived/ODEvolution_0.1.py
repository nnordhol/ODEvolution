...
class Experiment:
    ...
    def __init__(self, genotypes, cycles = 15, volume = 10, sample_volume = 0.1,dilution = 100, predilution = 4,
        ...
        self.after_cycle = np.array([])
        self.fractions = np.array([])
        self.total = np.array([np.sum(self.y0[:-1])])
    
    
    
    def from_geno(self, geno_att):
       if geno_att in Genotype().__dict__:
           return np.array([g.__dict__[geno_att] for g in self.genotypes])
       else:
    ...
    def conduct_once(self):
        ...
        self.sol = solve_ivp(pop_ivp,self.t_span,self.y0,args=([self.ode_args]),
        ...
        self.calc_fractions()
        self.total = np.append(self.total, np.array([np.sum(sol) for sol in self.sol.y[:-1].T]))
