...
##### TODO #######
# Option to switch off Poisson sampling
...
# Check influence of lower yield?
...
def growth(n, r, mumax, km, a = 0, ic50 = 1):
...
class Genotype:
    ...
    def __init__(self, mumax = 1.11, e = 5*10**-9, km = 0.25, n0 = 10**4, gid = 0, survival = 0.0004, mr=2*10**-10):
    ...
    def mutate(self, volume = 10):
        ...
        trade_off = stats.norm(0.461, 0.04).rvs(len(mu_effects))
...
class Experiment:
    ...
    def __init__(self, genotypes, volume = 10, sample_volume = 0.1, predilution = 4,
                 gluc_start = 20, t_span = [0,24], killing = True, poisson = True):
    ...
    def conduct_steps(self, cycles = 15, steps = 25, mutate = False, stop_after_first_extinct = False):
        ...
        for c in range(cycles):
            ...
            for t_span in t_spans:
                while any([g.stepdy[-1]>0 if g.stepdy.size>0 else True for g in self.genotypes]) or j == 0:
