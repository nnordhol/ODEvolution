...
##### TODO #######
# Make possible to let defined genotype emerge at certain timepoint. Then fit model to survival data.
...
# Make possible to start with a single genotype!
...
# Only simulate genotypes that are not extinct (extinct flag?)
# Only add data to genotypes that are not extinct!
# Stop simulation when glucose is exhausted 
...
class Genotype:
    ...
    def mutate(self, volume = 10):
        ...
        fitness_cutoff = 0.05
        # This underestimates the number of cells! We have to check difference between start and end of each 'step'
        dndt = (self.n[-1]-self.n[-2])*volume
        expected = dndt*self.mr
        mutants = stats.poisson(expected).rvs()
        #mutants = int(mutants*(10**-2)) # Include target size here?
        if mutants == 0:
            return
...
class Experiment:
    ...
    @jit
    def conduct_once(self, t_span = None):
    ...
    def conduct_steps(self, cycles = 15, steps = None, mutate = False):
        
        if steps is None:
            t_spans = [self.t_span]
        else:
            l,step = np.linspace(self.t_span[0], self.t_span[1], steps, retstep=True)
            t_spans = [[0,step]]*(steps-1)
        for c in range(cycles):
            for t_span in t_spans:
                self.conduct_once(t_span=t_span)
                if mutate:
                    mutants = [g.mutate(volume = self.volume) for g in self.genotypes if (g.n[-1]-g.n[-2])>=1]
